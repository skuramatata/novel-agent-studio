...
每次生成最多 2200 输出 token，单次请求 120 秒超时；网络/API 失败不自动重试、不发布半成品，可在 Actions 修复配置后重跑。五次上限统计已发布评论；删除机器人评论会影响计数，失败请求仍可能计费。默认不设全仓每日预算，公网大量新 Issue 仍可能产生费用，可在模型平台配置额度并按需停用工作流。
...
当前追问与原 Issue、历史评论分别传入；重跑旧评论时不读取后续评论作为上下文。默认简短回复，仅在需要时展开步骤。

`/ai 你好`、`/ai 谢谢` 等常见纯问候或确认由脚本直接回应，不检索、不调用 DeepSeek，但仍计入每个 Issue 的 5 次回复上限。混有技术问题的问候仍走分析流程。新问题按当前消息检索；短指代问题按关键词规则补充最近人类问题与原主题，该规则无法保证理解所有自然语言指代。

正文含 URL 或 Markdown 链接时，最多额外调用一次模型修正格式；仍不合规就失败且不发布。引用链接统一由脚本生成，不再插入“链接省略”占位文案。每个技术问题通常一次模型调用，格式修正时最多两次，每次最多 2200 输出 token。
