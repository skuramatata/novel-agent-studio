...
`review-context.mjs`按实际消息预算装配审稿文档。整份材料在阶段软额度内时沿用完整请求；超过时，若当前章可以完整装入，就每批共享当前章、只轮换历史原文；否则按段落分片，组合检查当前章段落与其余已提供原文，覆盖跨场景和历史证据。每个当前段落与所有已提供段落至少共同出现在一批中。批次数量随材料增加，创作任务不设本地总时长上限；调用预算仍有效，作者可手动停止，中断后复用已成功批次。
