...
(async () => {
  const root = process.cwd(),
    out = path.resolve("verification/memory-v2");
  ...
  try {
    ...
    assert.equal(task.memorySelection.version, 2);
    assert.ok(task.memorySelection.selectedRecords > 0);
    assert.ok(
      proposed.proposal.memory.entries.every(
        (e) =>
          e.dependencyScope === "chapter" && !Object.hasOwn(e, "prefixHash"),
      ),
    );
    ...
    await page
      .getByText("正在读取创作日志…", { exact: true })
      .waitFor({ state: "hidden" });
    await page
      .getByLabel("创作任务", { exact: true })
      .locator("option")
      .filter({ hasText: "暂无任务" })
    ...
    const report = {
      ...
      memorySelection: task.memorySelection,
      chapterScopedMemory: true,
    };
    await fs.writeFile(
